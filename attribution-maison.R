...
FetchBQData <- function(sourceProjet, sourceDataset, destinationTable, conversionID, conversionSQL, itemsSQL = "IF(trafficSource.isTrueDirect, 'Direct', channelGrouping)", window = 30) {
  sql <- sprintf("
    #legacySQL
...
# ------------- 2.1 - Identifier par quel dimension nous souhaitons effectuer l'attribution. -------------
          ...
          FROM
            TABLE_DATE_RANGE([%s:%s.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -%i, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -1, 'DAY'))  # %s = sourceProjet, %s = sourceDataset, %i = window
          WHERE
...
# ------------- 1 - Identifier les visiteurs qui ont effectué hier la conversion que nous voulons suivre. -------------
            SELECT
              ...
              CONCAT(fullVisitorId, STRING(visitId)) AS sessionID
            FROM
              TABLE_DATE_RANGE([%s:%s.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -1, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -1, 'DAY'))  # %s = sourceProjet, %s = sourceDataset
            WHERE
# ------------- 1.1 - Quel action définis une visite avec conversion. -------------
      ...
      GROUP BY
        sequences", itemsSQL, sourceProjet, sourceDataset, window, sourceProjet, sourceDataset, conversionSQL)
...
FetchBQData(
  "projectID",
  "datasetId",
  "modele_attribution_maison_campaign", 
  "Form-Prise-Rendez-Vous", 
  "hits.eventInfo.eventCategory = 'Formulaire' AND hits.eventInfo.eventAction = 'complete' AND hits.eventInfo.eventLabel = 'Prise de rendez-vous'", 
